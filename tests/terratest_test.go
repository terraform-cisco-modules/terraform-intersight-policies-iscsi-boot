...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-iscsi-b-%s", uniqueId)
	...
	adapter_moid := terraform.Output(t, terraformOptions, "adapter")
	ip_pool_moid := terraform.Output(t, terraformOptions, "ip_pool")
	moid := terraform.Output(t, terraformOptions, "moid")
	static_target_moid := terraform.Output(t, terraformOptions, "static_target")
	assert.NotEmpty(t, moid, "TF module moid output should not be empty")

	// Input variables for the API Verification
	vars2 := map[string]interface{}{
		"adapter_moid":       adapter_moid,
		"ip_pool_moid":       ip_pool_moid,
		"name":               instanceName,
		"static_target_moid": static_target_moid,
	}
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} iSCSI Boot Policy.",
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/vnic/IscsiBootPolicies/%s", moid), expectedJSONTemplate, vars2)
}
